#!/usr/bin/env node
/**
 * Deploy PrivaCred contract to Midnight Preprod network
 */
...
async function createProviders(walletCtx: ReturnType<typeof createWallet> extends Promise<infer T> ? T : never, zkConfigPath: string) {
  ...
  return {
    privateStateProvider: levelPrivateStateProvider({
      privateStateStoreName: 'priva-cred-private-state',
      walletProvider,
...
async function main() {
  console.log('\n╔══════════════════════════════════════════════════════════════╗');
  console.log('║         Deploy PrivaCred to Midnight Preprod                 ║');
  console.log('╚══════════════════════════════════════════════════════════════╝\n');
  ...
  const PrivaCred = await import(pathToFileURL(contractPath).href);
  const compiledContract = CompiledContract.make('priva-cred', PrivaCred.Contract).pipe(
    CompiledContract.withWitnesses({
      local_secret_key: (...args: any[]) => undefined,
      get_credential_data: (...args: any[]) => undefined,
    }),
